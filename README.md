...
만든다. 무료 쿼터 10,000유닛/일 이고 이 수집기는 1회당 약 1,600유닛을 쓴다(하루 2회 = 3,200).
해외 쿼리만 최신순·관련도순 두 번 돌기 때문이다 — 아래 '해외 영상이 안 잡힌다' 참고.
...
| Tobot X / Y / Z | 변신로봇 완구. 인니·베트남 채널에서 조회수 수백만이라 방치하면 유튜브 지표를 통째로 왜곡한다 |
| XYZ 축 (좌표계) | 갠트리·직교로봇·리니어 액추에이터. 해외 검색 결과의 최대 오탐원 |
| Crazy XYZ / XYZprinting | 각각 인도 실험 채널, 3D 프린터 회사(완구 로봇 Bolide) |
...
해외 판정은 한 단계 더 나뉜다. 라틴 문자권에서 'XYZ' 는 좌표계·플레이스홀더로 아무 데나
나오는 세 글자라 `INTL_EXCLUDE` 로 다른 뜻을 먼저 쳐내고, 그다음 `xyz` + 맥락을 본다.
유튜브는 편집을 안 거친 UGC 라 `strict: true` 로 한 번 더 좁혀서, generic 한 `robot` 만으로는
통과시키지 않고 DEUX·바리스타·카페·한국 같은 브랜드 고유 맥락을 요구한다. 뉴스에 같은 강도를
걸면 'XYZ to Equip China's Galaxea Robots' 같은 실제 보도가 탈락한다.
...
**해외 영상이 안 잡힌다** — 해외에서는 회사명(XYZ)보다 제품명(DEUX·Baris Brew)으로 불려서
쿼리에 제품명이 들어가야 한다. 그리고 해외 쿼리만 `order=date` 와 `order=relevance` 를 둘 다
돈다 — 해외 발행량이 적어 최신순 50건 창 밖으로 과거 보도가 밀려나기 때문이다. 실제로 최신순만
돌렸을 때 외신 6건 중 4건을 놓쳤다.

**유튜브 국내/해외 구분 기준** — 어느 쿼리에 걸렸는지가 아니라 내용으로 가른다. 한글이 있거나
공식 채널(`OFFICIAL_CHANNEL_IDS`) 이면 국내다. 'Baris Brew' 같은 제품명 쿼리에는 한국어 영상이
